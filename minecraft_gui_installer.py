...
class MinecraftServerGUI(tk.Tk):
    ...
    def create_tabs(self):
        ...
        tab_control.bind(
            "<<NotebookTabChanged>>",
            lambda e: self.update_dynamic_tabs(),
        )
    ...
    def create_main_tab(self):
        ...
        self.mc_version_cb.bind(
            "<<ComboboxSelected>>",
            self.on_minecraft_version_change,
        )
        ...
        self.server_type.bind(
            "<<ComboboxSelected>>",
            self.on_server_type_change,
        )

        self.mod_loader_version = ttk.Combobox(
            self.tab_main,
            state="readonly",
        )
        ...
        ttk.Button(
            self.tab_main,
            text="Install Server",
            command=self.install_server_click,
        ).pack(pady=20)
    ...
    def load_minecraft_versions(self):
        def run():
            try:
                ...
                self.filter_versions_by_server_type(
                    self.server_type.get().lower()
                )
    ...
    def filter_versions_by_server_type(self, server_type):
        if server_type == "vanilla":
            ...
            filtered_versions = self.all_minecraft_versions
        else:
            filtered_versions = [
                v
                for v in self.all_minecraft_versions
                if "snapshot" not in v and "preview" not in v
            ]
    ...
    def fetch_forge_versions(self, mc_version):
        def run():
            try:
                ...
                versions = [
                    v.text
                    for v in tree.findall(".//versioning/versions/version")
                ]
                filtered = [
                    v
                    for v in versions
    ...
    def create_java_tab(self):
        ttk.Label(
            self.tab_java,
            text="Check if Java is installed",
        ).pack(pady=10)
        ttk.Button(
            self.tab_java,
            text="Check Java",
            command=self.check_java,
        ).pack()
        ...
        ttk.Label(
            self.tab_java,
            text="Optional: Download & Install OpenJDK 17",
        ).pack(pady=(20, 0))
        ttk.Label(
            self.tab_java,
            text=(
                "Hinweis: Für die Installation werden Administratorrechte "
                "benötigt."
            ),
            foreground="red",
        ).pack(pady=(0, 10))
        ttk.Button(
            self.tab_java,
            text="Install Java",
            command=self.install_java_click,
        ).pack()

        self.java_progress = ttk.Progressbar(
            self.tab_java,
            orient='horizontal',
            length=300,
            mode='determinate',
    ...
    def create_plugins_tab(self):
        self.plugins_label = ttk.Label(
            self.tab_plugins,
            text=(
                "Plugins werden basierend auf dem gewählten Servertyp "
                "angezeigt."
            ),
        )
        self.plugins_label.pack(pady=20)

        self.link1 = ttk.Label(
            self.tab_plugins,
            text="Bukkit.org Plugins",
            foreground="blue",
            cursor="hand2",
        )
        self.link2 = ttk.Label(
            self.tab_plugins,
            text="SpigotMC Plugins",
            foreground="blue",
            cursor="hand2",
        )
        self.link1.bind(
            "<Button-1>",
            lambda e: self.open_url("https://dev.bukkit.org/bukkit-plugins"),
        )
        self.link2.bind(
            "<Button-1>",
            lambda e: self.open_url("https://www.spigotmc.org/resources/"),
        )
    ...
    def install_java_click(self):
        if not self.is_admin():
            messagebox.showinfo(
                ...
                (
                    "Die Java-Installation benötigt Administratorrechte. "
    ...
    def install_java(self, install_dir):
        jdk_url = (
            ...
            "jdk-17.0.7%2B7/OpenJDK17U-jdk_x64_windows_hotspot_17.0.7_7.zip"
        )
        ...
        try:
            ...
            messagebox.showerror(
                "Java Fehler",
                f"Fehler beim Download oder Entpacken:\n{e}",
            )
            self.java_progress['value'] = 0
    ...
    def install_server_click(self):
        ...
        threading.Thread(
            target=self.install_server,
            args=(install_dir, version, server_type, mod_loader_version),
        ).start()

    def install_server(
        self, install_dir, version, server_type, mod_loader_version=None
    ):
        try:
            ...
            if server_type == "vanilla":
                ...
                version_info = json.loads(
                    urllib.request.urlopen(version_data["url"]).read().decode()
                )
                jar_url = version_info["downloads"]["server"]["url"]
                self.status_label.config(
                    text="Vanilla Server wird heruntergeladen..."
                )
                ...
                jar_url = (
                    ...
                    f"versions/{version}/builds/latest/downloads/"
                    f"paper-{version}-latest.jar"
                )
                self.status_label.config(
                    text="Paper Server wird heruntergeladen..."
                )
                if not self.download_file(jar_url, "server.jar"):
                    self.status_label.config(text="Download fehlgeschlagen.")
                    return
                ...
                forge_installer_url = (
                    "https://maven.minecraftforge.net/net/minecraftforge/"
                    "forge/"
                    f"{mod_loader_version}/"
                    f"forge-{mod_loader_version}-installer.jar"
                )
                self.status_label.config(
                    text="Forge Installer wird heruntergeladen..."
                )
                ...
                self.status_label.config(
                    text="Forge Installer wird ausgeführt..."
                )
                subprocess.run(
                ...
                fabric_installer_url = (
                    ...
                    "0.11.2/fabric-installer-0.11.2.jar"
                )
                self.status_label.config(
                    text="Fabric Installer wird heruntergeladen..."
                )
                if not self.download_file(
                    fabric_installer_url,
                ...
                self.status_label.config(
                    text="Fabric Installer wird ausgeführt..."
                )
                subprocess.run(
                    [
                        "java",
                ...
                if not os.path.exists(
                    os.path.join(install_dir, "fabric-server-launch.jar")
                ):
            ...
            with open("start.bat", "w") as f:
                ...
                if java_path:
                    f.write(
                        f'"{java_path}" -Xmx{ram} -jar server.jar nogui\n'
                    )
                else:
                    f.write(
                        f'java -Xmx{ram} -jar server.jar nogui\n'
                    )
                f.write("pause\n")
            ...
            messagebox.showinfo(
                "Erfolg",
                "Minecraft Server erfolgreich installiert!",
            )
        except Exception as e:
