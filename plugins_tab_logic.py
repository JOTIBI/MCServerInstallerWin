...
class PluginsTab(ttk.Frame):
    def __init__(self, parent, server_type_getter):
        ...
        self.notice_label = ttk.Label(
            self,
            text="Wähle einen Servertyp aus und öffne dann diesen Tab."
        )
        ...
        self.link1.bind(
            "<Button-1>",
            lambda e: webbrowser.open("https://dev.bukkit.org/bukkit-plugins")
        )
        self.link2.bind(
            "<Button-1>",
            lambda e: webbrowser.open("https://www.spigotmc.org/resources/")
        )
    ...
    def update_content(self):
        ...
        if server_type in ["paper", "spigot"]:
            ...
            ttk.Label(
                self,
                text="Mods verwalten im Reiter 'Mods'."
            ).pack(pady=20)
        else:
            ttk.Label(
                self,
